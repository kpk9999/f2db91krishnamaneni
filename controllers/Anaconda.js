...
// Handle Anaconda delete on DELETE.
exports.Anaconda_delete = async function (req, res) {
    console.log("delete " + req.params.id)
    try {
        result = await Anaconda.findByIdAndDelete(req.params.id)
...
exports.Anaconda_view_one_Page = async function (req, res) {
    console.log("single view for id " + req.query.id)
    try {
        result = await Anaconda.findById(req.query.id)
        res.render('Anacondadetail',
            { title: 'Anaconda Detail', toShow: result });
    }
    catch (err) {
        res.status(500)
        res.send(`{'error': '${err}'}`);
...
exports.Anaconda_create_Page = function (req, res) {
    console.log("create view")
    try {
        res.render('Anacondacreate', { title: 'Anaconda Create' });
    }
    catch (err) {
        res.status(500)
        res.send(`{'error': '${err}'}`);
...
exports.Anaconda_update_Page = async function (req, res) {
    console.log("update view for item " + req.query.id)
    try {
        let result = await Anaconda.findById(req.query.id)
        res.render('Anacondaupdate', { title: 'Anaconda Update', toShow: result });
    }
    catch (err) {
        res.status(500)
        res.send(`{'error': '${err}'}`);
...
exports.Anaconda_delete_Page = async function (req, res) {
    console.log("Delete view for id " + req.query.id)
    try {
        result = await Anaconda.findById(req.query.id)
        res.render('Anacondadelete', {
            title: 'Anaconda Delete', toShow:
                result
        });
    }
    catch (err) {
